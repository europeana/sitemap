...
/**
 * Load the socks proxy configuration from the provided properties files. Note that we load files manually and not
 * use default Spring-Boot functionality here because this needs to be done early during start-up before any connections
 * are set-up.
 *
...
public class SocksProxyConfig {
    ...
    /**
     * Load the provided properties files. Files are read in order they are provided, so the same properties defined
     * later will override previous provided properties. Each file should be on the class path.
     *
     * @param propertiesFileNames properties files to read. If a file cannot be found a warning is logged
     */
